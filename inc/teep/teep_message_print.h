...
teep_err_t teep_print_update(const teep_update_t *teep_update, uint32_t indent_space, uint32_t indent_delta, const char *ta_public_key);
...
teep_err_t teep_print_message(const teep_message_t *msg, uint32_t indent_space, uint32_t indent_delta, const char *ta_public_key);
